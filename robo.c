...
void VulcanGun(char solid)
{
  ...
  for (i = 0; i < 5; i++) {
    ...
    // Additional code for bullets
    for (i=0;i<5;i++)
    {
    glTranslatef(0.0, 0.0, 0.1);
    gluQuadricDrawStyle(qobj, GLU_FILL); // Set the draw style to solid for bullets
    gluSphere(qobj, 0.052, 16, 10); // Draw a bullet as a sphere
	}
	

    glTranslatef(0.0, -0.3, -2.2); // Adjust the translation to match bullet position
  }
  glEndList();
}
...
void display(void)
{
 ...
 glLoadName(TEXTID);
 glColor3f(1,1,0);
 DrawTextXY(-2.5,0.2,2.0,0.0015,"3br20cs168");
 DrawTextXY(-2.5,0.5,2.0,0.0015,"saniya");
 DrawTextXY(2.5,2.2,-2.0,0.0025,"3br20cs169");
 DrawTextXY(2.5,2.7,-2.0,0.0030,"tanish");
  glFlush();
  glutSwapBuffers();
